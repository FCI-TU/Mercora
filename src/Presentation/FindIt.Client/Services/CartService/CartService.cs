...
public class CartService(IHttpClientFactory _httpClientFactory, ILocalStorageService localStorageService) : ICartService
{
	...
	public async Task<string> PlaceOrder(string cartId)
	{
		var response = await httpClient.PostAsync($"api/payment/checkout-session/{cartId}", null);
		
		if (response.IsSuccessStatusCode)
		{
			Cart = new CartResponse(new Guid().ToString());
